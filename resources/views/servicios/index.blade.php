...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <a href="{{ route('load.form') }}" id="new-servicio" type="button" class="btn btn-info col-4"> <i
                    class="fas fa-plus-circle text-white fa-2x"></i>Servicios</a>
        </div>
    </div>
            ...
            <tbody>
                ...
                @foreach ($servicio as $serv)
                    <tr>
                        ...
                        <td>{{ $serv->nombre_resp ?? ' ' }} {{ $serv->primerap_resp ?? '' }} {{ $serv->segap_resp ?? '' }}
                        </td>
                        <td>{{ $serv->servicio ?? '' }}</td>
                        <td>{{ $serv->monto ?? '0' }}</td>
                        <td>{{ $serv->fur }}</td>

                        @if ($serv->tipo != 'EXTERNO GRATUITO')
                            <td><button class="btn btn-warning verificar_pago" data-id="{{ $serv->serv_id }}"
                                    value="{{ $serv->fur }}"><i
                                        class="fas fa-check-square fa-2x  accent-blue "></i></button>
                            <td>
                         @else
                         <td> <P>Sin costo</P>
                          <td>
...
@section('js')
    <script>
        ...
        $(document).on('click', '.anular_cm', function(e) {
            e.preventDefault();
            var fur = $(this).attr('id');
