...
class MantenimientoController extends Controller
{
    ...
    public function savePay(Request $request)
    {
        ...
        if ($request->isJson()) {
            $this->validate($request, [
                ...
                'fecha_ingreso_nicho.required' => 'El campo fecha de ingreso es requerido',
            ...
            if ($existeNicho != null) {
                ...
                $ni = new Nicho();
                $id_nicho= $ni->InsertNichoPartial(
                    $id_cuartel,
                    $id_bloque,
                    $request->nro_nicho,
                    $request->fila,
    ...
    public function pagoMantenimientoCM(Request $request)
    {
        ...
        if ($request->isJson()) {
            ...
            $obj = new ServicioNicho;
            $response = $obj->GenerarFurCMant(
                $request->ci,
                $request->nombrepago,
                $request->paternopago,
                $request->maternopago,
                $request->domicilio,
                $request->codigo_unidad,
                $servicio_hijos,
                $cantidades,
                $cajero,
                $request->resp_id,
